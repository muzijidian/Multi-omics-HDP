...
# Split combined_data into data starting with g__ and s__
g_data <- combined_data %>% filter(str_starts(name, "g__"))
s_data <- combined_data %>% filter(str_starts(name, "s__"))

# Find the order of names starting with g__ where the p-value is < 0.1 in any of PE_T1, GH_T1, or PEvsGH_T1
...
# Find the order of the remaining names starting with g__
...
# Find the order of names starting with s__ where the p-value is < 0.1 in any of PE_T1, GH_T1, or PEvsGH_T1
...
# Find the order of the remaining names starting with s__
...
if (level == 'g'){
  if(state=="T1"){
    ...
    unique_phylum <- c("Bacteroidetes", "Firmicutes", "Proteobacteria", "Actinobacteria") 
...
if(level=='s'){
  ...
  # --- Modification 2: Generate heatmap object, but do not save yet (filename = NA) ---
  heatmap <- pheatmap(
    beta_data_orig,
    cluster_rows = FALSE,
    cluster_cols = FALSE,
    ...
    # Key point: Set to NA so pheatmap returns an object instead of saving directly
    filename = NA, 
  ...
  # --- Modification 3: Modify the plot object to set row names to italic ---
  # Find the part corresponding to "row_names" in the heatmap object
  row_label_idx <- which(heatmap$gtable$layout$name == "row_names")
  
  # Force the fontface of this part to italic
  heatmap$gtable$grobs[[row_label_idx]]$gp$fontface <- "italic"
  
  # --- Modification 4: Manually save the modified heatmap ---
...
# Merge the combined results of the first and second files again
...
# Find the order of names where data_source is 'ITS' and the p-value is < 0.1 in any of PE_T2, GH_T2, or PEvsGH_T2
g_priority_names <- g_data %>%
  group_by(name) %>%
  filter(any(source %in% c("PE_T2", "GH_T2", "PEvsGH_T2") & pv_adj_FDR < 0.1)) %>%
  ungroup() %>%
  distinct(name) %>%
  arrange(name) %>%  # Reorder by name
  pull(name)
...
# Find the order of the remaining names where data_source is 'ITS'
g_remaining_names <- g_data %>%
  filter(!(name %in% g_priority_names)) %>%
  distinct(name) %>%
  arrange(name) %>%  # Reorder by name
...
# Find the order of names where data_source is 'MGG' and the p-value is < 0.1 in any of PE_T1, GH_T1, or PEvsGH_T1
s_priority_names <- s_data %>%
  group_by(name) %>%
  filter(any(source %in% c("PE_T1", "GH_T1", "PEvsGH_T1") & pv_adj_FDR < 0.1)) %>%
  ungroup() %>%
...
# Find the order of the remaining names where data_source is 'MGG'
s_remaining_names <- s_data %>%
  filter(!(name %in% s_priority_names)) %>%
  distinct(name) %>%
  arrange(name) %>%  # Reorder by name
...
if(data_type=="ITS"){
  ...
  if(state=="T23"){
    unique_phylum <- c("Bacteroidota", "Firmicutes", "Proteobacteria", "Actinobacteriota", "Cyanobacteria") 
    phylum_colors <- setNames(colorRampPalette(brewer.pal(5, "Set3"))(length(unique_phylum)), unique_phylum)
  }
  if(state=="all"){
    unique_phylum <- c("Bacteroidetes", "Firmicutes", "Proteobacteria", "Actinobacteria", "Ascomycota", "Basidiomycota")
