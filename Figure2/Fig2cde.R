...
library(cowplot)
library(purrr)
library(rstatix)   # For significance testing
library(boot)      # For Bootstrap
library(ggpubr)    # For stat_pvalue_manual and stat_compare_means

setwd('/Users/mzjd/Documents/HDP-multiomics/haonan_code')
source('../R_code/myUtils.R')
source('help_func.R') # Load help_func in advance
set.seed(123)
...
# ==============================================================================
# 2. Plot the first graph (p_pcoa1_box) - Grouped by time period
# ==============================================================================
df_pcoa$group <- factor(df_pcoa$group, levels = c("NP", "GH", "PE"))
col_group = colorspace::lighten(c('#7a94ba',"#f8c672",'#d73027'), amount = 0.1)

# Pairwise Wilcoxon tests within each period using rstatix, with BH correction
stat_test_pcoa <- df_pcoa %>%
  group_by(period) %>%
  wilcox_test(PCOA1 ~ group, p.adjust.method = "BH") %>%
  add_significance("p.adj") %>%
  add_xy_position(x = "period", dodge = 0.8) # Calculate P-value marker position after boxplot dodge
...
p_pcoa1_box <- ggplot(df_pcoa, aes(x = period, y = PCOA1)) +
  ...
  # Add significance markers (hide non-significant lines to keep the plot clean)
...
# ==============================================================================
# 3. Plot the second graph (p_trend) - Placed in the middle
# ==============================================================================
df_trend = data.frame() 
for (t in c('T1','T2','T3')){
  for (g in c('PE', 'GH', 'NP')){
...
# ==============================================================================
# 4. Plot the third graph (p1) - Placed on the far right
# ==============================================================================
...
# ==============================================================================
# 5. Combine plots and save (Patchwork)
# ==============================================================================
# Adjust the width ratio of the three plots using plot_layout
p_plot = p_pcoa1_box + p_trend + p1 + plot_layout(widths = c(2.3, 2.3, 1.4))

# Adjusted canvas size to fit the new ratio
...
bcdist_ITS <- bcdist_ITS %>% 
  # 1. Remove rows where HDP is NA
  filter(!is.na(HDP)) %>% 
  # 2. Generate group column based on rules
  mutate(
    group = case_when(
      HDP == 1 & preeclampsia == 1 ~ "PE",
      HDP == 1 & preeclampsia == 0 ~ "GH",
...
# Separate tests for each period
for (t in c('T1','T2','T3')){
  set.seed(1111)
...
# Separate tests for each disease group
...
ggsave('Anti_graph/figs2_bray_PCOA_ITS.pdf', width = 6.2, height = 6.2)
